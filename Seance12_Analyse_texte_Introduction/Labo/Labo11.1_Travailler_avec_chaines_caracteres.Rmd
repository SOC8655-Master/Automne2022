---
title: 'Labo 11.1: Travailler avec les chaines de caractères'
...
restaurants <- read_csv("/Users/visseho/OneDrive - UQAM/Cours/Bases_donnees/restaurants.csv",
...
Dans d'autres langues, "chaîne" fait souvent référence à une *séquence* d'informations numériques, comme chaînes binaires (par exemple "01110000 01101111 01101111 01110000"). Nous les utilisons rarement dans R.
...
`sep =` contrôle ce qui se passe lorsque vous combinez en entrée les vecteurs que vous donnez à `paste()`. Vous obtiendrez toujours un vecteur. `collapse =` contrôle si/comment ils passent d'un vecteur à une seule chaîne.
...
- Supprimez les espaces ou remplissez-les
- Effectuer des tâches liées à **la correspondance de modèles**: Detect, locate, extract, match, replace, split.
...
**Les expressions régulières** ou **regex** sont la façon dont nous décrivons les modèles que nous recherchons dans le texte d'une manière qu'un ordinateur peut comprendre. Nous écrivons une **expression**, l'appliquons à une entrée de chaîne, puis peut faire des choses avec **correspond ** que nous trouvons.
...
- **Métacaractères** soyons flexibles dans la description des modèles:
  - barre oblique inversée `\`, caret `^`, signe dollar `$`, point `.`, pipe `|`, point d'interrogation `? `, astérisque `*', signe plus `+`, parenthèses `(` et `)`, crochets `[` et `]`, accolades `{` et `}`
...
Faites juste attention à ne pas charger un long vecteur / variable entier ou cela pourrait planter RStudio alors qu'il essaie de rendre une page massive!


# Combien de lignes ont des nombres non-206?
