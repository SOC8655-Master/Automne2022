...
La fonction kmeans() implémente l'algorithme k-means dans R. La fonction a différents arguments, mais le premier argument x prend une matrice d'observations à laquelle on applique l'algorithme k-means. 

Les principaux arguments de la fonction kmeans() incluent "centers" (le nombre de clusters), "iter.max" (le nombre maximum d'itérations) et "nstart" (le nombre de centroïdes initiaux choisis au hasard). Il est recommandé de spécifier l'argument nstart afin que l'algorithme soit exécuté plusieurs fois avec des valeurs de départ différentes (la fonction kmeans() rapporte les meilleurs résultats).
...
Les objets de sortie, kmean_out, sont des listes qui contiennent divers éléments concernant les résultats de l'application de l'algorithme k-means. Ils incluent iter (un entier représentant le nombre d'itérations jusqu'à la convergence, qui est atteinte lorsque les affectations de cluster ne changent plus), cluster (un vecteur de l'appartenance au cluster résultant) et "centers" (une matrice de centroïdes de cluster).

- La convergence est atteinte au bout de 3 itérations. Le nombre maximal d'itérations par défaut est de 10. Si la convergence n'est pas atteinte, l'argument iter.max doit être spécifié comme un nombre supérieur à 10.
...
## Tableau avec les top word de chaque cluster
...
Comme mentionné précédemment, la paternité de certains des documents fédéralistes est inconnue. Nous utiliserons les 66 essais attribués à Hamilton ou à Madison pour prédire la paternité des 11 articles contestés. Étant donné que chaque article fédéraliste traite d'un sujet différent, vous allez vous concentrer sur l'utilisation des articles, des prépositions et des conjonctions. En particulier, vous allez analyser la fréquence d'utilisation des 10 mots suivants par les deux auteurs: **although, always, commonly, consequently, considerable, enough, there, upon, while, whilst**. Ces mots sont sélectionnés sur la base de l'analyse présentée dans l'article académique qui a inspiré cet exercice. En conséquence, vous devez utiliser le corpus sans la stemmisation. 

- Créer une base de données avec les identifiants des essais de chaque auteur avec le nom des auteurs. Cette nouvelle base de données que vous appelez known_essays doit avoir deux variables (document et author) et com
...
author_data <-
  ...
  mutate(author2 = case_when(.$author == "Hamilton" ~ 1,
                             .$author == "Madison" ~ -1,
                             TRUE ~ NA_real_)) %>% 
  select(author, author2, upon,  there, consequently, whilst)
...
Les résultats ci-dessous montrent que même lorsque la procédure de validation croisée est utilisée, le modèle continue de classer parfaitement la paternité de chaque essai.
