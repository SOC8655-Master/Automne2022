...
3. Je répondrai à vos questions sur ce devoir jusqu'au **mardi 8 octobre**

4. Éviter le **plagiat**. Éviter de vous copier.

6. Ce devoir est noté sur 50 et compte pour 5% de la note finale.
...
Nous commençons par calculer *le taux brut de natalité* (TBN) pour une période donnée. Le TBN est défini comme suit:
 $$ 
  ...
  $$
Calculez le TBN pour chaque période, séparément pour le Kenya et la Suède. 
...
Une solution intermédiare au TBN est le calcul du *taux général de fécondité (TGF)*. Celui-ci se calcule par:
 $$ 
  ...
  $$
Calculer ce taux pour le Kenya. Pourquoi cela vous parait déjà meilleur que le TBN? Interpréter les résultats. Quel est le problème du taux général de fécondité.
...
tfa <- ggplot(KSW, aes(x = age, y = tfa)) +
...
isf <- ggplot(isf) +
  geom_col(aes(x = period, y = isf, fill = country)) +
  facet_grid(~country)

isf

ggsave("isf.png", plot = isf)
```
