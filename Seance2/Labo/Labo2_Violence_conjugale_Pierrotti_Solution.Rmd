...
Ces statistiques nous donnent une description de l'ensemble de l'echantillon. On peut toutefois aussi présenter les informations sous formes graphiques. Présenter quatre graphiques `Barplot` donnant la distribution de chacune des quatre variables d'intérêt selon la région d'étude? Commenter les résultats?
...
ggplot(dhs_ipv) +
  geom_point(aes(x = sec_school, y = beat_burnfood, color = region)) +
  geom_smooth(aes(x = sec_school, y = beat_burnfood, method = "lm", se = FALSE, color = region))
...
table(dhs_subset$mean_bf, dhs_subset$region)
